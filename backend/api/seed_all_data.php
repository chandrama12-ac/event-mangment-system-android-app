...
try {
    ...
    for ($i = 1; $i <= 20; $i++) {
        ...
        $stmt->execute([
            ':title' => $title,
            ':slug' => $slug,
            ':description' => "This is a comprehensive demo event for " . $categories[$i % count($categories)] . ". Join us for an exciting experience!",
    ...
    for ($i = 1; $i <= 100; $i++) {
        ...
        $stmt->execute([
            ':title' => $title,
            ':slug' => $slug,
            ':description' => "Deep dive into $topic. This educational session covers advanced concepts and practical applications.",
